function imData=bigread2_partial(path_to_file, indices_i, indices_j, sframe, num2read)
...
% Jason Moore 2024, v1.4 - Adds the ability to load in only segments of each
% image. Technically it loads in the entire frame and then throws away
% everything the user doesn't want. Could be sped up by doing the algebra
% to only load in the necessary data
...
if strcmpi(ext,'.tiff') || strcmpi(ext,'.tif')
    ...
    fr_h = length(indices_i);
    fr_w = length(indices_j);

    bd=info.BitDepth;
    byteSize = bd/8;
    frameOffset = ((indices_i(1)-1)*he_w + (indices_j(1)-1))*byteSize;
    ...
    for i=1:numFrames
        ofds(i)=info(i).StripOffsets(1) + frameOffset;
        %ofds(i)
    end
...
%     imData=cell(1,framenum);
    ...
    imData = zeros(fr_h, fr_w, framenum, form);
    ...
    if strcmpi(form,'uint16') || strcmpi(form,'uint8')
        if(bo)
            for cnt = sframe:lastframe
                ...
                tmp1 = fread(fp, [fr_w fr_h], sprintf('%d*%s',fr_w,form),byteSize*(he_w-fr_w), 'ieee-be');
                tmp1 = tmp1';                
            ...
            for cnt=sframe:lastframe
                % cnt;                
                fseek(fp,ofds(cnt),'bof');
                tmp1 = fread(fp, [fr_w fr_h], sprintf('%d*%s',fr_w,form),byteSize*(he_w-fr_w), 'ieee-le');
                tmp1 = tmp1';                
                imData(:,:,cnt-sframe+1)=cast(tmp1,form);
            end
        end
        ...
        elseif strcmpi(form,'double')
            if(bo)
                for cnt = sframe:lastframe
                    % cnt;                
                    fseek(fp,ofds(cnt),'bof');
                    tmp1 = fread(fp, [fr_w fr_h], sprintf('%d*%s',fr_w,form),byteSize*(he_w-fr_w), 'ieee-be.l64');
                    tmp1 = tmp1';                
                ...
                for cnt = sframe:lastframe
                    % cnt;                
                    fseek(fp,ofds(cnt),'bof');
                    tmp1 = fread(fp, [fr_w fr_h], sprintf('%d*%s',fr_w,form),byteSize*(he_w-fr_w), 'ieee-le.l64');
                    tmp1 = tmp1';                
                    imData(:,:,cnt-sframe+1)=cast(tmp1,'single');                   
                end
            end
